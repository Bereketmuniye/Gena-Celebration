...
const CultureChat: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-[500px] md:h-full glass-card rounded-2xl overflow-hidden shadow-2xl">
      <div className="p-4 md:p-6 bg-stone-100/80 border-b border-stone-200 flex items-center justify-between">
        <h2 className="text-base md:text-xl font-serif text-stone-900 font-bold">The Chronicler's Study</h2>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[95%] md:max-w-[85%] p-3 md:p-4 rounded-2xl text-[11px] md:text-sm leading-relaxed font-medium ${m.role === 'user'
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-stone-100 p-3 md:p-4 rounded-2xl border border-stone-200 animate-pulse text-stone-700 font-bold text-[11px] md:text-sm">
              Channelling history...
            </div>
          </div>
        )}
      ...
      <div className="p-4 md:p-6 bg-stone-50/50 border-t border-stone-200">
        <div className="flex space-x-2 md:space-x-3">
          <input
            ...
            className="flex-1 bg-white border border-stone-300 rounded-xl px-3 md:px-4 py-2.5 md:py-3 focus:outline-none focus:ring-2 focus:ring-amber-600/50 transition-all text-xs md:text-sm text-stone-950 font-medium placeholder:text-stone-400"
          />
          <button
            onClick={handleSend}
            disabled={isLoading}
            className="bg-amber-700 hover:bg-amber-800 disabled:opacity-50 text-white px-4 md:px-6 py-2.5 md:py-3 rounded-xl font-bold transition-all shadow-lg text-xs md:text-sm"
          >
            Send
          </button>
        </div>
