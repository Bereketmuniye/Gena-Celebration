    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 h-full">
      <div className="lg:col-span-1 space-y-6">
        <div className="glass-card p-4 md:p-6 rounded-2xl">
          ...
          <p className="text-stone-800 text-xs md:text-sm mb-4 md:mb-6 font-medium">Ethiopian cuisine is a communal experience. Discover the secrets of our traditional spices and fermentation.</p>
            ...
            {featuredDishes.map((d, i) => {
              ...
              return (
                <button
                  ...
                  className={`text-xs md:text-sm py-2 md:py-3 rounded-xl border transition-all font-bold ${colors[i % colors.length]}`}
                >
                  {d}
                </button>
              );
          ...
          <div className="space-y-2 md:space-y-3">
            <input
              ...
              className="w-full bg-white border border-stone-300 rounded-xl px-3 md:px-4 py-2 md:py-3 focus:outline-none focus:ring-2 focus:ring-amber-600/50 text-xs md:text-sm text-stone-950 font-medium placeholder:text-stone-400"
            />
            <button
              onClick={() => handleFetchRecipe()}
              className="w-full bg-red-700 hover:bg-red-800 text-white py-2 md:py-3 rounded-xl font-bold transition-all shadow-lg text-sm"
            >
              Get Recipe
            </button>
          </div>
      ...
      <div className="lg:col-span-2 glass-card rounded-2xl p-6 md:p-8 overflow-y-auto relative min-h-[400px] md:min-h-[500px]">
        {isLoading ? (
          ...
          <div className="space-y-6 md:space-y-8 animate-fadeIn">
            <div>
              <h1 className="text-3xl md:text-4xl font-serif text-amber-800 mb-2 font-bold">{recipe.name}</h1>
              <div className="h-1 w-16 md:w-24 ethiopian-gradient rounded-full"></div>
            </div>

            <section>
              <h3 className="text-lg md:text-xl font-serif text-stone-900 mb-2 md:mb-3 italic font-bold">Historical Context</h3>
              <p className="text-stone-800 leading-relaxed text-xs md:text-sm font-medium">{recipe.history}</p>
            </section>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
              <section>
                <h3 className="text-base md:text-lg font-bold text-emerald-800 mb-3 md:mb-4 uppercase tracking-wider">Ingredients</h3>
              ...
              <section>
                <h3 className="text-base md:text-lg font-bold text-yellow-700 mb-3 md:mb-4 uppercase tracking-wider">Method</h3>
